...
| `azure-devops` | repo-owned (no upstream) | Azure DevOps via the `az` CLI: build URL / PR number → run → timeline → failed task → log → root cause; pipeline-run and PR inspection; Windows / Git Bash / pipeline-YAML pitfalls. Commands verified against the Movares `Dataplatform` org; needs `az` + `azure-devops` extension + `jq` |
